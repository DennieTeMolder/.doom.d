...
(defun dtm-org-link-string (link)
  "Return the raw string for org-element LINK."
  (replace-regexp-in-string (rx (or "[[" (seq "]" (* nonl)))) ""
                            (buffer-substring-no-properties
                             (org-element-property :begin link)
                             (org-element-property :end link))))

;; REVIEW add a variable to disable conversion and return expanded tag?
...
The zeroth element is total number of converions
...
(defun dtm/org-link-as-png-convert (&rest _)
  ...
  (let* ((conversions (dtm-org-link-as-png-conversions))
         ...
         (n-total (- (car conversions) n-broken))
         (missing (nth 1 conversions))
         (outdated (nth 2 conversions))
         response)
    (cond ((not conversions)
           (message "Org[as_png]: No conversions detected"))
          ((not (or missing outdated))
           (message "Org[as_png]: All %s conversions are up to date (%s broken)"
                    n-total n-broken))
          (t (setq response 'ask)))
    (when (eq 'ask response)
      (setq response
            (thread-first
